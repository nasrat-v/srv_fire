...
void ImageAdditionner::drawNumberEntitiesOnImage(const std::vector<Entity> &entities, cv::Mat &img)
{
    ...
    for (auto &entity : entities)
    {
        ...
        if (entity.getType() == Entity::entityType::STATIC)
            cv::putText(img, /*std::to_string(nb)*/"static", entity.getCenterPositions().back(), cv::FONT_HERSHEY_SIMPLEX, fontScale, SCALAR_GREEN, intFontThickness);
        else
            cv::putText(img, /*std::to_string(nb)*/"move", entity.getCenterPositions().back(), cv::FONT_HERSHEY_SIMPLEX, fontScale, SCALAR_GREEN, intFontThickness);
    }
    nb++;
}
