...
bool PacketHandler::getDataBaseEmebedded(std::string embededId) {
    ...
    if (embededId == "-1") {
        ...
        ApiAnswer.erase(ApiAnswer.end() - 2);
        ApiAnswer.at(ApiAnswer.size() - 2) = ']';
        ApiAnswer.at(ApiAnswer.size() - 26) = '[';
        ApiAnswer.erase(ApiAnswer.end() - 27);
...
void PacketHandler::setLen(socklen_t len) {
    PacketHandler::len = len;
}

void PacketHandler::set_udpCliInf(const sockaddr_in &_udpCliInf) {
    PacketHandler::_udpCliInf = _udpCliInf;
}
